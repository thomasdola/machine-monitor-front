...
class PoliciesBox extends React.Component {
    ...
    _handleOnAdd(){
        const policies = _filter(this.state.rowsBuffer, ({isSelected}) => isSelected === true);
    ...
    _buildRowContent(row, {editRoleActionAllowed, deletePolicyActionAllowed, editPolicyActionAllowed}){
        ...
        return (
            <div style={{position: 'relative'}} className="mm-collapse-row-content">
                ...
                <section className="list">
                    ...
                    <div className="value">
                        {actions.map(({id,name}) => [
                            <Tag intent={Intent.SUCCESS} className="bp3-minimal" key={id}>{name}</Tag>,
                ...
                <ButtonGroup style={{position: 'absolute', top: '10px', right: '5px'}} vertical className="bp3-small">
                    {editPolicyActionAllowed && (
                        <Link to={`/roles/policies/${uuid}/edit${search}`}
                          className="bp3-button bp3-minimal bp3-intent-primary bp3-small bp3-icon-edit"/>
                    )}
                    <Button
    ...
    render(){
        ...
        const rows = this.state.rows.map(function(row){
            return (
                <div key={row.uuid} className="mm-collapse-row">
                    <div
                        className={`mm-collapse-row-label mm-collapse-row-label-plain
        ...
        return (
            <div style={{height: '100%'}} className="mm-collapse">
                {this.props.filterable && (
                    <div className="mm-collapse-header">
                        <input
                            value={this.state.filterQuery || ''}
                            onKeyDown={this._onKeyPress}
                            onChange={this._onSearchQueryChange}
                            type="text" className="bp3-input bp3-fill mm-small" placeholder="filter..." />
                        <Button
                            loading={this.props.editingGroup}
                            onClick={this._handleOnAdd}
                            disabled={!canAdd}
                            text={'add'} className={`bp3-small ${canAdd ? 'bp3-intent-primary' : ''}`}/>
                    </div>
                )}
...
export default withRouter(connect()(PoliciesBox));