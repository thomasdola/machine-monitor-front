    ...
    render(){
        ...
        return (
            <Select
                ...
                popoverProps={{ usePortal: false, lazy: false, position: Position.TOP, minimal: true, className: 'mm__select__popover' }}
                onItemSelect={(item) => this._onItemSelected(item)}>
                <Button
                    onClick={() => this.setState({popOpen: !this.state.popOpen})}
                    disabled={disabled}
                    className={`bp3-fill mm-multi-select ${small && 'bp3-small mm-multi-select-small'}`}
